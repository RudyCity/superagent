...
2. SKILL CHECK (MANDATORY FIRST STEP): Before doing anything else, scan the INSTALLED AGENT SKILLS list in your system prompt. Identify relevant skills for this task (including dynamically loaded skills from active internal hooks under \`internal-hooks/\` or \`ih\`) and read their SKILL.md using its absolute path from that list via a file-reading tool. Pass relevant skill paths to your Subagents so they also follow the skill workflows.
