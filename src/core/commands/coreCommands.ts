  ...
  execute(args, ctx) {
    ctx.addLine({
      ...
      content: [
        ...
        "               /ssh status            - Show host, workspace path, and allowed paths",
        "               /ssh expand <path>     - Whitelist an extra absolute path (e.g. /var/www/html)",
        "               /ssh allowed           - List all whitelisted extra paths",
        "               Tip: also supported at connect time via ?allow= URL param",
        ...
        "  /daemon   - Manage autonomous background daemon & cron scheduler",
        "              Usage: /daemon [start|stop|status|list|add|remove|run]",
        "  /skills   - List installed agent skills and templates (/skills [list|synthesize])",
        ...
        "                     Usage: /setting-rmemory [on|off|provider <local|openai>|model <name>|dimensions <number>]",
        "  /help     - Show this help",
        "  /quit     - Exit the app",
        "",
        "Shortcuts:",
...
export const initCommand: SlashCommand = {
  ...
  async execute(args, ctx) {
    ...
    try {
      ...
      if (settings.contextWindowLimit > 0) limit = settings.contextWindowLimit;
