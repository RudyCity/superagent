    ...
    if (updates.ACTIVE_PROVIDER) {
        ...
        if (!updates[`${prefix}_API_KEY`]) {
            try {
                ...
                const matchedProfile = providers.find((p) => p.id?.toLowerCase() === activeProvider || p.name?.toLowerCase() === activeProvider);
                ...
                const fallbackProfile = matchedProfile && matchedProfile.apiKey && matchedProfile.apiKey.trim() !== ""
                    ? matchedProfile
                    : providers.find((p) => (p.provider || "").toLowerCase() === fallbackProviderType && p.apiKey && p.apiKey.trim() !== "");
