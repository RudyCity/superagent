...
export function getProviderOptionsList(list) {
    ...
    const templatesToShow = defaultTemplates.filter((t) => {
        ...
        else if (lowerT.includes("openai") && !lowerT.includes("custom"))
            nameToMatch = "openai";
        else if (lowerT.includes("anthropic") && !lowerT.includes("custom"))
            nameToMatch = "anthropic";
        else if (lowerT.includes("custom") && lowerT.includes("openai"))
            nameToMatch = "custom";
        else if (lowerT.includes("custom") && lowerT.includes("anthropic"))
            nameToMatch = "custom-anthropic";
...
export function getTierModelWithProvider(mode, tier) {
    ...
    const pick = (...configs) => configs.find((c) => c?.model);
    let tierConfig;
    if (key === "master") {
        ...
        // subagentDefault → superagent → master (multi only)
        tierConfig = pick(preset.models.subagentDefault, preset.models.superagent, m === "multi" ? preset.models.master : undefined);
    }
    else {
        // Named subagent — fallback chain: subagentDetails[name] → subagentDefault → superagent → master (multi only)
        tierConfig = pick(preset.models.subagentDetails?.[key], preset.models.subagentDefault, preset.models.superagent, m === "multi" ? preset.models.master : undefined);
    }
    if (!tierConfig?.model)
        return "";
