...
export function isAnthropicCompatible(baseUrl, modelName) {
...
export function getModelInstanceForString(modelStr) {
    const config = getConfig();
    if (!modelStr) {
    ...
    if (isProviderPrefix) {
        ...
        if (prefix === "anthropic") {
            ...
            modelName = rest;
            baseUrl = undefined;
            const anthropicProfile = modelConfig.providers.find((p) => p.provider === "anthropic" && p.apiKey && p.apiKey.trim() !== "");
            if (anthropicProfile) {
            ...
            modelName = rest;
            baseUrl = undefined;
            const openaiProfile = modelConfig.providers.find((p) => p.provider === "openai" && p.apiKey && p.apiKey.trim() !== "");
            if (openaiProfile) {
            ...
            provider = "custom";
            modelName = rest;
            const customProfile = modelConfig.providers.find((p) => p.provider === "custom" && p.apiKey && p.apiKey.trim() !== "");
            if (customProfile) {
            ...
            const matchedProvider = modelConfig.providers?.find(p => p.id?.toLowerCase() === prefix || p.name?.toLowerCase() === prefix);
            if (matchedProvider) {
                ...
                if (!apiKey || apiKey.trim() === "") {
                    const fallbackProfile = modelConfig.providers.find((p) => p.id !== matchedProvider.id && (p.provider || "").toLowerCase() === typeLower && p.apiKey && p.apiKey.trim() !== "");
                    if (fallbackProfile) {
                ...
                const openrouterProfile = modelConfig.providers.find((p) => (p.provider === "openrouter" || p.provider === "custom") && p.baseUrl?.includes("openrouter.ai") && p.apiKey && p.apiKey.trim() !== "");
 ...
 */
function isValidTierConfig(tierConfig, providers) {
    if (!tierConfig)
        return false;
    if (!tierConfig.model || tierConfig.model.trim() === "")
        return false;
    ...
    if (tierConfig.providerProfileId && tierConfig.providerProfileId.trim() !== "") {
        const resolvedProvider = providers.find((p) => p.id === tierConfig.providerProfileId);
        if (!resolvedProvider)
            return false;
    }
    return true;
 ...
 */
function resolveSubagentTierConfig(subagentType, subagentDetails, subagentDefault, providers) {
...
export function getModelConnectionDetailsForTier(tier, depth, subagentType, isSingleMode) {
...
export function getModelInstanceForTier(tier, depth, subagentType, isSingleMode) {
