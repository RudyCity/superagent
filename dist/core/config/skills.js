...
import os from "os";
import { getPackageRootDir } from "./paths.js";
...
export function getInstalledSkills() {
                    ...
                    for (const [k, v] of Object.entries(lockData.skills)) {
                        if (v && typeof v === "object" && "source" in v && typeof v.source === "string") {
                            ...
                            if ("skillPath" in v && typeof v.skillPath === "string") {
                                const pathBase = path.basename(path.dirname(v.skillPath)).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    ...
    searchDirs.push(path.join(os.homedir(), ".superagent-r", "skills"), path.join(packageRootDir, "skills"), path.join(packageRootDir, ".agents", "skills"));
    ...
    function processSkillFile(skillMdPath, folderName, defaultAuthor) {
        try {
            ...
            const hasNameDuplicate = skills.some(s => s.name.toLowerCase() === name.toLowerCase() &&
