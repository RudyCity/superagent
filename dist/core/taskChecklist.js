 ...
 */
export function getTaskHistoryPath(taskPath) {
    return taskPath.replace(/_task\.md$/, "_task_history.md");
}
export function parseChecklistTasks(content) {
...
export async function readChecklistTasks(taskPath) {
    try {
 ...
 */
export async function allTasksCompleted(taskPath) {
    const result = await readChecklistTasks(taskPath);
    if (result.missing || result.tasks.length === 0)
        return false;
 ...
 */
export async function archiveCompletedTasks(taskPath) {
    const result = await readChecklistTasks(taskPath);
    if (result.missing || result.tasks.length === 0)
        return [];
 ...
 */
function trimHistoryRounds(content, maxRounds) {
    const lines = content.split(/\r?\n/);
    const headerIndices = [];
    ...
    if (headerIndices.length <= maxRounds)
        return content;
 ...
 */
export async function readTaskHistory(taskPath) {
    const historyPath = getTaskHistoryPath(taskPath);
    try {
 ...
 */
export async function readTaskHistoryGrouped(taskPath) {
    const historyPath = getTaskHistoryPath(taskPath);
    try {
        const content = await fs.readFile(historyPath, "utf-8");
 ...
 */
function parseHistoryRounds(content) {
    const lines = content.split(/\r?\n/);
    const rounds = [];
