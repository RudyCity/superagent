...
import { getModelPresets, saveModelPreset, applyModelPreset, getContextWindowLimit, fetchAndCacheModels, } from "../config.js";
...
function formatModelWithProvider(tier, config) {
    if (!tier?.model)
        return "(use default)";
...
function getActiveModelInfo(isMulti) {
...
function formatModelList(info, isMulti) {
    let list = "";
    if (isMulti) {
        ...
        if (args) {
            try {
                ...
                if (parts.length >= 2) {
                    ...
                    if (["master", "superagent", "subagent", "depth0", "depth1", "depth2", "dept0", "dept1", "dept2"].includes(firstWord) ||
