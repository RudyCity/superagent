...
import { subagentInstances, backgroundTasks, notifyTasksChanged, } from "../tools.js";
        ...
        if (activeList.length === 0) {
            ...
            activeList.forEach(([id, inst], index) => {
                ...
                if (inst.status === "completed" && inst.result) {
                    const snippet = inst.result.length > 60 ? inst.result.slice(0, 57) + "..." : inst.result;
...
export const processesCommand = {
    ...
    async execute(args, ctx) {
        ...
        const windowTasks = taskList.filter(([, t]) => t.isDetachedWindow);
        const bgTasks = taskList.filter(([, t]) => !t.isDetachedWindow);
        if (windowTasks.length > 0) {
            lines.push("│  🖥️  TERMINAL WINDOWS (detached)");
            for (const [id, task] of windowTasks) {
                const label = task.windowLabel || task.command.split(" ")[0];
...
export const goalCommand = {
    ...
    async execute(args, ctx) {
        ...
        try {
            ...
            let existing = "";
            try {
                existing = await fsPromises.readFile(scratchPath, "utf-8");
            }
            catch { }
