...
function buildPreview(content, maxLen) {
...
function formatAgentTag(tag) {
    if (!tag)
        return "";
    const parts = [];
    if (tag.tier && tag.tier !== "unknown")
        parts.push(tag.tier);
    if (tag.subagentType)
        parts.push(tag.subagentType);
    ...
    execute(args, ctx) {
            ...
            for (let i = startIndex; i < messages.length; i++) {
                ...
                if (rawContent.length === 0) {
                    if (msg.toolCalls && msg.toolCalls.length > 0) {
                        rawContent = `(tool calls: ${msg.toolCalls.map((tc) => tc.name).join(", ")})`;
                    }
                    else if (msg.toolResults && msg.toolResults.length > 0) {
                        rawContent = `(tool results: ${msg.toolResults.map((tr) => tr.name).join(", ")})`;
                    }
                    else {
                        rawContent = "(empty)";
                    }
        ...
        if (trimmed.startsWith("view ")) {
            ...
            if (pin.agentTag) {
                ...
                if (pin.agentTag.subagentType)
                    lines.push(`  Subagent    : ${pin.agentTag.subagentType}`);
                if (pin.agentTag.worktreePath)
                    lines.push(`  Worktree    : ${pin.agentTag.worktreePath}`);
                if (pin.agentTag.workingDirectory)
                    lines.push(`  Working Dir : ${pin.agentTag.workingDirectory}`);
                lines.push("");
            }
        ...
        if (trimmed === "last") {
            ...
            for (let i = messages.length - 1; i >= 0; i--) {
                if (messages[i].role === "user") {
                    ...
                    try {
                        const sessionPath = agent.getCurrentHistoryFilePath();
                        addToKnowledge({ id: msgId, role: msg.role, content: stringContent, timestamp: msg.timestamp, pinnedAt: now, originalIndex: i, agentTag, toolCalls: msg.toolCalls, toolResults: msg.toolResults }, sessionPath, agent.workingDirectory);
                    }
                    catch { /* non-critical */ }
        ...
        try {
            const sessionPath = agent.getCurrentHistoryFilePath();
            addToKnowledge({ id: msgId, role: msg.role, content: stringContent, timestamp: msg.timestamp, pinnedAt: now, originalIndex: idx, agentTag, toolCalls: msg.toolCalls, toolResults: msg.toolResults }, sessionPath, agent.workingDirectory);
        }
        catch { /* non-critical */ }
        const preview = buildPreview(stringContent, 80);
        ctx.addLine({
