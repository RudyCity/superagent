...
import { formatPresetValue, getPresetLabel, findPreset } from "./types.js";
import { getGlobalConfigDir } from "../config.js";
import { backgroundTasks, notifyTasksChanged } from "../tools.js";
import { killProcessTree } from "../tools/shellTools.js";
function normalizeCwd(cwdPath) {
...
export const terminalCommand = {
    ...
    async execute(args, ctx) {
        ...
        if (args.toLowerCase() === "init") {
            ...
            ctx.agent?.sendMessage("USER COMMAND: /terminal init\n\n" +
                ...
                "5. Write the final configuration back to the local project file `.superagent-r/terminal-presets.json` using a file writing tool. Confirm to the user once it is completed.").catch((err) => {
        ...
        if (args.toLowerCase() === "bg" || args.toLowerCase().startsWith("bg ")) {
            ...
            (async () => {
                ...
                const resolveBgPreset = (val, label) => {
                    bgPresetName = label;
                ...
                if (!fsCb.existsSync(tasksLogDir))
                    fsCb.mkdirSync(tasksLogDir, { recursive: true });
                const logPath = path.join(tasksLogDir, `${taskId}.log`);
                try {
                    fsCb.writeFileSync(logPath, "");
                }
                catch { /* ignore */ }
                let shellPath = true;
                if (process.platform === "win32") {
                ...
                proc.all?.on("data", (data) => {
                    ...
                    if (task.output.length > 1000)
                        task.output.shift();
                    try {
                        fsCb.appendFileSync(logPath, text);
                    }
                    catch { /* ignore */ }
                });
                proc.on("close", (code) => {
                    ...
                    try {
                        fsCb.appendFileSync(logPath, exitMsg);
                    }
                    catch { /* ignore */ }
                    notifyTasksChanged();
                });
        ...
        const loadPresetsAndRun = async () => {
            ...
            let isPreset = false;
            let presetName = "";
            const runCmd = async (singleCmd, labelOverride) => {
                let commandStr = "";
                let runCwd = cwd;
                let runEnv = { ...process.env };
                ...
                if (!fsCb.existsSync(logDir))
                    fsCb.mkdirSync(logDir, { recursive: true });
                ...
                try {
                    fsCb.rmSync(closeSignalPath, { force: true });
                }
                catch { /* ignore */ }
                ctx.addLine({
                    type: "system",
                    ...
                    if (process.platform === "win32") {
                        ...
                        const handleViewerExit = () => {
                            if (!task.hasExited) {
                                ...
                                try {
                                    fsCb.appendFileSync(logPath, closeMsg);
                                }
                                catch { /* ignore */ }
                                try {
                        ...
                        execa("x-terminal-emulator", ["-e", `bash -c "tail -f '${safeLog}'"`], { detached: true, stdio: "ignore", reject: false }).unref();
                    }
                }
                ...
                ctx.addLine({
                    type: "system",
                    content: `[TERMINAL CONTEXT] ID: ${taskId} | Label: ${windowLabel}\n` +
                        `  Command : ${commandStr}\n` +
                        `  Log     : ${logPath}\n` +
