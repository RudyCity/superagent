...
import url from "url";
async function pingPort(port, host = "localhost") {
    return new Promise((resolve) => {
...
async function pingHttp(targetUrl) {
    return new Promise((resolve) => {
        try {
...
export const runBackgroundProcessTool = {
    ...
    async execute(args, cwd, signal) {
        ...
        const startProcess = async () => {
            ...
            currentProc.on("close", async (code) => {
                hasExited = true;
                exitCode = code;
            ...
            while (elapsedMs < timeoutMs) {
                ...
                if (args.healthCheckUrl || args.healthCheckPort) {
                    ...
                    if (args.healthCheckUrl) {
                        healthPassed = await pingHttp(args.healthCheckUrl);
                    }
