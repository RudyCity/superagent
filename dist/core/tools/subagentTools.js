import { subagentTypes, subagentInstances, superagentInstances, notifySubagentsChanged, getMasterAgent, getActiveQuestionHandler, appendMasterLog } from "./state.js";
 ...
 */
function extractSubagentReport(agentInstance) {
 ...
 */
function resolveSubagentInstance(identifier) {
...
export const defineSubagentTool = {
...
export const invokeSubagentTool = {
    ...
    async execute(args, cwd, signal) {
        ...
        function formatSubagentArgs(subArgs) {
        ...
        async (toolCall, _desc) => {
            const cmd = (toolCall.args.command || "").trim();
...
export const manageSubagentsTool = {
    ...
    async execute(args, cwd, signal) {
        ...
        if (action === "list") {
            const lines = ["Defined Subagent Types:"];
        ...
        if (action === "violations") {
            const lines = ["Subagent Violations Report:"];
