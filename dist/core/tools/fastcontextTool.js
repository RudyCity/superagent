...
async function resolveFastContextCredentials() {
    ...
    const pick = (...configs) => configs.find((c) => c.value?.model);
    const resolved = pick({ name: "researcher", value: activePreset.models.subagentDetails?.researcher }, { name: "subagentDefault", value: activePreset.models.subagentDefault }, { name: "superagent", value: activePreset.models.superagent }, { name: "master", value: mode === "multi" ? activePreset.models.master : undefined });
    ...
    if (prefix) {
        providerProfile = providers.find((p) => p.id?.toLowerCase() === prefix || p.name?.toLowerCase() === prefix);
    }
    if (!providerProfile && providerProfileId) {
        providerProfile = providers.find((p) => p.id === providerProfileId);
        if (!providerProfile) {
            const staleId = providerProfileId.toLowerCase();
            providerProfile = providers.find((p) => p.id?.toLowerCase() === staleId || p.name?.toLowerCase() === staleId || p.provider?.toLowerCase() === staleId);
        }
    }
    ...
    if (!providerProfile || !providerProfile.apiKey || providerProfile.apiKey.trim() === "") {
        const anyWithKey = providers.find((p) => p.apiKey && p.apiKey.trim() !== "");
        if (anyWithKey) {
...
export const fastcontextTool = {
    ...
    parameters: {
        ...
        properties: {
            ...
            exclude: {
                type: "string",
                description: "Comma-separated glob patterns to exclude from all searches (e.g. " +
                    "'node_modules,dist,.git,*.min.js'). Applied to Grep and Glob tool calls.",
            },
            maxFileSizeKb: {
                type: "number",
                description: "Skip files larger than this size in KB when reading (default: 512). " +
                    "Reduces token usage on binary or generated files.",
            },
            noCache: {
                type: "boolean",
                description: "If true, bypass the query result cache and always run a fresh exploration. " +
    ...
    async execute(args, cwd, signal) {
        ...
        const maxFileSizeKb = args.maxFileSizeKb || 512;
        ...
        try {
            ...
            if (exitedWithError) {
                ...
                for (const line of stderrRaw.split("\n")) {
                    try {
                        ...
                        if (evt.event === "error" && evt.text)
                            errorEvents.push(evt.text);
                    }
                    catch { /* not JSON */ }
                }
                ...
                if (errorEvents.length > 0) {
                    ...
                    if (stderrRaw)
                        parts.push(`stderr: ${stderrRaw.slice(0, 500)}`);
                    if (!output && !stderrRaw)
            ...
            if (msg.includes("timed out") || msg.includes("timeout")) {
                return (`FastContext timed out after ${timeoutMin} minutes. ` +
