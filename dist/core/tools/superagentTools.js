...
import { superagentInstances, notifySuperagentsChanged, getMasterAgent, getActiveQuestionHandler, addHistoricalSuperagentTokens, appendMasterLog, } from "./state.js";
...
`;
// ─── invoke_superagent ────────────────────────────────────────────────────────
export const invokeSuperagentTool = {
    name: "invoke_superagent",
    description: "Spawn a Superagent to develop a feature in an isolated git worktree on its own branch. " +
    ...
    parameters: {
        ...
        properties: {
            ...
            baseBranch: {
                type: "string",
                description: "Optional: the branch to create the worktree FROM. Use this when the Superagent " +
            ...
            mode: {
                type: "string",
                enum: ["full", "patch"],
                description: "Execution mode. 'full' (default): creates an isolated git worktree for the Superagent. " +
    ...
    async execute(args, cwd, signal) {
        ...
        const logs = [];
        let lastTextIdx = -1;
        function appendToThinkingNode(text) {
            if (lastTextIdx === -1) {
                logs.push(`[THINK] `);
        ...
        const agentInstance = new Agent((event) => {
            if (event.type === "text") {
        ...
        async (_toolCall, _desc) => {
            const cmd = (_toolCall.args.command || "").trim();
        ...
        async (question, options) => {
            ...
            if (master && typeof master.answerQuestionAsMaster === "function") {
                ...
                const answer = await master.answerQuestionAsMaster(question, options, {
                    source: "superagent",
                    role,
                    task,
                    branch,
            ...
            return options[0] ?? "";
        }, systemPrompt, superagentToolset, worktreePath);
        ...
        if (wait) {
            ...
            run().catch(() => { });
            return (`Superagent "${role}" spawned in background.\n` +
                `  ID:       ${superagentId}\n` +
                `  Branch:   ${branch}\n` +
...
// ─── await_superagents ────────────────────────────────────────────────────────
export const awaitSuperagentsTool = {
    name: "await_superagents",
    description: "Wait for all active Superagents to finish their work. " +
    ...
    async execute(args, _cwd, signal) {
        ...
        const running = [...superagentInstances.values()].filter((i) => i.status === "running");
        const paused = [...superagentInstances.values()].filter((i) => i.status === "paused");
        ...
        while (true) {
            if (signal?.aborted)
                return "Aborted while waiting for Superagents.";
            ...
            const hasRunning = [...superagentInstances.values()].some((i) => i.status === "running");
            const hasPaused = [...superagentInstances.values()].some((i) => i.status === "paused");
            if (!hasRunning) {
                if (hasPaused) {
        ...
        // Build summary
        const lines = ["All Superagents finished:\n"];
        for (const inst of superagentInstances.values()) {
            ...
            if (inst.result) {
                const preview = inst.result.length > 300
...
// ─── merge_superagents ────────────────────────────────────────────────────────
export const mergeSuperagentsTool = {
    name: "merge_superagents",
    description: "Merge all completed Superagent branches into the current branch using AI-assisted conflict resolution. " +
        "Only callable by the Master Agent (depth 0). " +
    ...
    async execute(args, cwd, signal) {
        ...
        const completed = [...superagentInstances.values()].filter((i) => i.status === "completed");
        ...
        const results = [`Merging ${completed.length} Superagent branch(es):\n`];
        for (const inst of completed) {
            // Discover which files were changed in the feature branch
            let changedFiles = [];
            try {
                const { stdout } = await execa("git", ["diff", "--name-only", `HEAD...${inst.branch}`], { cwd });
...
async function cleanupWorktreeRobust(worktreePath, logs, cwd) {
...
// ─── manage_superagents ───────────────────────────────────────────────────────
export const manageSuperagentsTool = {
    ...
    async execute(args, cwd, signal) {
        ...
        const superagentIds = args.superagentIds;
        ...
        if (action === "list") {
            const lines = ["Active Superagent Instances:"];
        ...
        if (action === "violations") {
            const lines = ["Superagent Violations Report:"];
...
// ─── define_superagent ────────────────────────────────────────────────────────
export const defineSuperagentTool = {
...
export const sendMessageToSuperagentTool = {
    ...
    async execute(args, cwd, signal) {
        ...
        const superagentId = args.superagentId;
        ...
        if (isPaused) {
            ...
            const logsList = logs || [];
            let lastTextIdx = -1;
            const appendToThinkingNode = (text) => {
                if (lastTextIdx === -1) {
                    logsList.push(`[THINK] `);
...
//# sourceMappingURL=superagentTools.js.map