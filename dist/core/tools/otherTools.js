    ...
    async execute(args, cwd, signal) {
        ...
        if (hasQuestionsArray) {
            ...
            if (currentTier === "superagent" || currentTier === "subagent") {
                ...
                const role = currentAgent.subagentType || currentAgent.tier || "?";
                    ...
                    if (master && typeof master.answerQuestionAsMaster === "function") {
                        try {
                            const selected = await master.answerQuestionAsMaster(q.question, q.options, {
                                source: currentTier,
                                role,
                                typeName: currentAgent.subagentType,
                            });
...
// ─── load_pinned_session ─────────────────────────────────────────────────────
export const loadPinnedSessionTool = {
...
export const searchPinnedKnowledgeTool = {
...
export const listPeerSuperagentsTool = {
...
export const managePlanTool = {
    ...
    async execute(args, cwd, signal) {
        const action = args.action;
        const planContentInput = args.planContent;
        const sessionId = args.sessionId;
        const { agentLocalStorage } = await import("../agent.js");
        const currentAgent = agentLocalStorage.getStore();
        let planPath;
        ...
        const injectMasterAgentTasks = (tasks) => {
        ...
        const syncTasks = async (planText) => {
