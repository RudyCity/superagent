 ...
 */
export function addToKnowledge(pinned, sourceSessionPath, workingDirectory) {
    const store = readStore();
    // Check for duplicate (same session + same original index)
    const existing = store.entries.find((e) => e.sourceSessionPath === sourceSessionPath && e.preview === (pinned.content || "").substring(0, 200));
    if (existing) {
 ...
 */
export function removeFromKnowledge(id) {
    const store = readStore();
 ...
 */
export function removeKnowledgeByPin(sourceSessionPath, contentPreview) {
    const store = readStore();
    const preview = contentPreview.substring(0, 200);
    const idx = store.entries.findIndex((e) => e.sourceSessionPath === sourceSessionPath && e.preview === preview);
    if (idx === -1)
        return false;
    store.entries.splice(idx, 1);
    writeStore(store);
 ...
 */
export function updateKnowledgeTag(sourceSessionPath, contentPreview, tag) {
    const store = readStore();
    const preview = contentPreview.substring(0, 200);
    const entry = store.entries.find((e) => e.sourceSessionPath === sourceSessionPath && e.preview === preview);
    if (!entry)
        return false;
    entry.tag = tag;
    writeStore(store);
 ...
 */
export function removeSessionFromKnowledge(sourceSessionPath) {
    const store = readStore();
 ...
 */
export function getKnowledgeProjects() {
    const store = readStore();
 ...
 */
export function formatKnowledgeForPrompt(entries, maxEntries = 10, maxContentChars = 2000) {
    if (entries.length === 0)
        return "";
    const lines = [];
 ...
 */
export function loadSessionFromKnowledge(sourceSessionPath) {
    try {
 ...
 */
export function getSessionTranscript(sourceSessionPath, maxChars = 30000) {
...
//# sourceMappingURL=pinnedKnowledge.js.map