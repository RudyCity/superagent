...
describe("File tools", () => {
    ...
    it("should replace file content using replaceFileContentTool and show a diff summary", async () => {
        ...
        const result = await tool?.execute({ filePath: "temp_unit_test.txt", targetContent: "line B", replacementContent: "line B Edited", startLine: 1, endLine: 3 }, process.cwd());
    ...
    it("should preserve CRLF line endings in replace_file_content, multi_replace_file_content, and apply_patch", async () => {
        ...
        try {
            ...
            const rResult = await replaceTool?.execute({ filePath: "temp_crlf_test.txt", targetContent: "line B", replacementContent: "line B\nline B2", startLine: 1, endLine: 3 }, process.cwd());
...
describe("Scheduler and Subagent tools", () => {
    ...
    it("should define, invoke, list, and message subagents with delegation depth checks", async () => {
        ...
        const defRes = await defSub?.execute({ name: "test_subagent", description: "testing", systemPrompt: "system text" }, process.cwd());
        expect(defRes).toContain("Subagent type \"test_subagent\" defined");
        const invRes = await invSub?.execute({ typeName: "test_subagent", role: "reviewer", prompt: "review this code", wait: false }, process.cwd());
        ...
        const parentAgent = { delegationDepth: 2 };
        await agentLocalStorage.run(parentAgent, async () => {
            const result = await invSub?.execute({ typeName: "test_subagent", role: "reviewer", prompt: "nested subagent call" }, process.cwd());
