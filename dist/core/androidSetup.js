...
export async function isAndroidCliInstalledGlobally() {
...
export function getLocalAndroidCliPath() {
    const isWin = process.platform === "win32";
    if (isWin) {
...
export async function isAndroidCliInstalledLocally() {
    try {
...
let cachedRgInstalledGlobally = null;
export async function isRgInstalledGlobally() {
    if (cachedRgInstalledGlobally !== null)
...
export function getLocalRgPath() {
    const isWin = process.platform === "win32";
    return path.join(getLocalBinDir(), isWin ? "rg.exe" : "rg");
}
let cachedRgInstalledLocally = null;
export async function isRgInstalledLocally() {
    if (cachedRgInstalledLocally !== null)
...
export async function ensureRgInstalled() {
    try {
...
export async function isCurlInstalledGlobally() {
    const isWin = process.platform === "win32";
    try {
...
export function getLocalCurlPath() {
    const isWin = process.platform === "win32";
    return path.join(getLocalBinDir(), isWin ? "curl.exe" : "curl");
}
export async function isCurlInstalledLocally() {
    try {
...
export async function ensureCurlInstalled() {
    try {
...
export async function ensureAndroidCliInstalled() {
    try {
