 ...
 */
export function parseConflictHunks(content) {
    const regex = /<<<<<<<[^\n]*\r?\n([\s\S]*?)\r?\n=======\r?\n([\s\S]*?)\r?\n>>>>>>>[^\n]*/g;
    const hunks = [];
    let match;
 ...
 */
export async function resolveFileConflicts(filePath, model // LLM Model instance
) {
 ...
 */
function detectConflictMarkers(filePath, content) {
    const errors = [];
 ...
 */
function detectDuplicateAdjacentLines(filePath, content) {
    const errors = [];
    const lines = content.split(/\r?\n/);
        ...
        if (prev === curr && prev.length > 0) {
            ...
            if (!isIgnored) {
                ...
                if (consecutiveDupCount >= maxAllowedConsecutive) {
                    errors.push(`${filePath}:${i + 1} — Suspicious duplicate adjacent lines (${consecutiveDupCount + 1}x): "${curr.substring(0, 80)}"`);
                }
            }
        }
        else {
 ...
 */
function detectLineMerging(filePath, content) {
    const warnings = [];
    const lines = content.split(/\r?\n/);
    for (let i = 0; i < lines.length; i++) {
        ...
        if (semicolonCount >= 2) {
            warnings.push(`${filePath}:${i + 1} — Possible line merging: ${semicolonCount + 1} statements on one line`);
        }
        ...
        if (tagMatches.length >= 3 && line.length > 80) {
            warnings.push(`${filePath}:${i + 1} — Possible line merging: ${tagMatches.length} opening tags on one line`);
        }
    }
    return warnings;
}
 ...
 */
function detectDuplicateAttributes(filePath, content) {
    const errors = [];
    const lines = content.split(/\r?\n/);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((match = attrPattern.exec(line)) !== null) {
            ...
            if (attrName.startsWith("data-") || attrName === "style")
                continue;
            if (attrs.includes(attrName)) {
                errors.push(`${filePath}:${i + 1} — Duplicate attribute "${attrName}" on same line: "${line.trim().substring(0, 80)}"`);
            }
            attrs.push(attrName);
        }
    }
 ...
 */
async function diffSanityCheck(cwd, branchName) {
    const warnings = [];
    try {
        // Get the diff between the merge base and the branch (expected changes)
        const { stdout: branchDiff } = await execa("git", ["diff", "--stat", `HEAD...${branchName}`], { cwd });
        // Get the staged diff (what merge actually produced)
        const { stdout: stagedDiff } = await execa("git", ["diff", "--stat", "--cached"], { cwd });
        // Parse line counts from --stat output
        const parseStatLines = (stat) => {
        ...
        if (branchInsertions >= 10 && stagedInsertions > branchInsertions * 3) {
            warnings.push(`Diff sanity check: staged diff has ${stagedInsertions} insertions but branch only has ${branchInsertions}. ` +
 ...
 */
async function runProjectValidation(cwd) {
    const errors = [];
    const warnings = [];
 ...
 */
export async function validatePostMerge(cwd, branchName, changedFiles) {
    const allErrors = [];
    const allWarnings = [];
 ...
 */
export function tryLineBasedResolution(filePath) {
    try {
        if (!fs.existsSync(filePath))
            return false;
        let content = fs.readFileSync(filePath, "utf-8");
...
export class MasterAgent {
     ...
     */
    async mergeBranch(branchName, targetFiles) {
        ...
        try {
            ...
            if (validation.warnings.length > 0) {
                console.warn(`[MERGE VALIDATION WARN] Branch "${branchName}":\n` +
                ...
                if (allResolved) {
                    ...
                    try {
                        ...
                        if (validation.valid) {
                            ...
                            console.warn(`[MERGE LINE-BASED] Branch "${branchName}" merged with line-based resolution.\n` +
