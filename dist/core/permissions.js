...
function resolveNormalizedPath(fp, baseDir) {
    let normalized = fp;
...
export function normalizeAndCheckSubpath(childPath, parentPath) {
 ...
 */
export function isPathInWorktree(filePath, worktreePath) {
 ...
 */
export function isSuperagentOutOfBounds(toolCall, worktreePath) {
    ...
    if (!checkedTools.includes(toolCall.name))
        return false;
    ...
    for (const fp of candidatePaths) {
        ...
        if (!isPathInWorktree(fp, worktreePath))
            return true;
    }
    return false;
}
 ...
 */
export function isToolCallOutOfBounds(toolCall, workspacePath) {
    const args = toolCall.args || {};
    const candidatePaths = [
        args.filePath,
        args.file_path,
 ...
 */
export function isModelConfigAccess(toolCall, workspacePath) {
    const args = toolCall.args || {};
    const rootConfig = resolveNormalizedPath(getRootConfigDir());
    ...
    for (const fp of candidatePaths) {
        ...
        const resolved = isAbs
            ? resolveNormalizedPath(fp)
            : resolveNormalizedPath(fp, workspacePath);
        if (normalizeAndCheckSubpath(resolved, modelConfigPath))
            return true;
    }
 ...
 */
export function isSensitiveEnvFileAccess(toolCall) {
    const args = toolCall.args || {};
    const candidatePaths = [
        args.filePath, args.file_path, args.TargetFile, args.path,
        args.AbsolutePath,
...
export function getToolDescription(toolCall) {
    ...
    const fp = (args.filePath ?? args.file_path ?? args.TargetFile ?? "(missing)");
    /** Safe string fallback helper for description interpolation */
    const s = (v) => (v !== undefined && v !== null ? String(v) : "(missing)");
...
function isErrorLikeToolResult(result) {
    const trimmed = result.trim();
    return (/^Error(?:\b|:)/i.test(trimmed) ||
        /^Error reading file:/i.test(trimmed) ||
        /^Git worktree error:/i.test(trimmed) ||
        /^Exit code:\s*[1-9]\d*/i.test(trimmed));
}
export async function executeToolCall(toolCall, cwd, signal) {
