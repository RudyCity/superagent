...
import { createCheckpoint, listCheckpointsForSession, restoreCheckpoint, deleteCheckpointsForSession, getGitSha, } from "./checkpoints.js";
describe("checkpoints", () => {
    const sessionFilePath = path.join(getGlobalConfigDir(), "history", "test_session.json");
    ...
    afterEach(async () => {
        ...
        for (const suffix of [
            ...
            try {
                await fsPromises.unlink(sessionFilePath.replace(/\.json$/, suffix));
            }
            catch { }
        }
    });
    ...
    it("createCheckpoint should save a checkpoint file and return valid data", async () => {
        const checkpoint = await createCheckpoint(sessionFilePath, "Test Checkpoint", sampleMessages, "IDLE");
        ...
        const expectedPath = path.join(checkpointsDir, `checkpoint_${checkpoint.timestamp}.json`);
        createdFiles.push(expectedPath);
        expect(fs.existsSync(expectedPath)).toBe(true);
        const savedContent = JSON.parse(fs.readFileSync(expectedPath, "utf-8"));
    ...
    it("createCheckpoint should capture plan/task/walkthrough file contents", async () => {
        ...
        const checkpoint = await createCheckpoint(sessionFilePath, "With Plan", sampleMessages, "PLANNING_PENDING");
        createdFiles.push(path.join(checkpointsDir, `checkpoint_${checkpoint.timestamp}.json`));
    ...
    it("listCheckpointsForSession should return checkpoints sorted newest-first", async () => {
        ...
        createdFiles.push(path.join(checkpointsDir, `checkpoint_${c1.timestamp}.json`), path.join(checkpointsDir, `checkpoint_${c2.timestamp}.json`), path.join(checkpointsDir, `checkpoint_${c3.timestamp}.json`));
    ...
    it("createCheckpoint should prune older checkpoints beyond 30", async () => {
        ...
        const newChk = await createCheckpoint(sessionFilePath, "Newest", sampleMessages, "IDLE");
        createdFiles.push(path.join(checkpointsDir, `checkpoint_${newChk.timestamp}.json`));
        // After pruning, total should be <= 30
        const list = await listCheckpointsForSession(sessionFilePath);
        expect(list.length).toBeLessThanOrEqual(30);
        // Oldest files should have been deleted
        const oldestPath = path.join(checkpointsDir, `checkpoint_${oldTimestamps[0]}.json`);
