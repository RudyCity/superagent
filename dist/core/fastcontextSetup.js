 ...
 */
export function isFastContextReady() {
 ...
 */
export function runFastContextSetup() {
    if (isFastContextReady()) {
        return;
    }
    if (!existsSync(SETUP_SCRIPT)) {
        console.log("[FastContext] Setup script not found. FastContext tool will be unavailable.\n" +
    ...
    try {
        if (process.platform === "win32") {
            ...
            execSync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${SETUP_SCRIPT}"`, {
                cwd: PROJECT_ROOT,
                stdio: "inherit",
...
//# sourceMappingURL=fastcontextSetup.js.map