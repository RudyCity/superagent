...
import { executeToolCall, getToolDescription, isDangerousCommand, MODIFYING_TOOLS, isSuperagentOutOfBounds, } from "./permissions.js";
                ...
                for (const tc of toolCalls) {
                    ...
                    if (MODIFYING_TOOLS.includes(tc.name)) {
                        ...
                        if (isPlanFile) {
                            ...
                            const missing = [];
                            if (!hasTitle)
                                missing.push("Main Title (e.g., '# Goal Description')");
                            if (!hasProposedChanges)
                                missing.push("Proposed Changes section ('## Proposed Changes')");
                            if (!hasVerificationPlan)
                                missing.push("Verification Plan section ('## Verification Plan')");
                            if (!hasAutomatedTests)
                                missing.push("Automated Tests sub-section ('### Automated Tests')");
                            if (!hasManualVerification)
                                missing.push("Manual Verification sub-section ('### Manual Verification')");
                            if (this.tier === "master") {
                    ...
                    if (tc.name === "bash" || tc.name === "run_command" || tc.name === "run_background_process") {
                        ...
                        if (isDangerousCommand(tc.args.command)) {
                    ...
                    const toolResult = await executeToolCall(tc, effectiveCwd, this.abortController?.signal);
                    if (this.abortController?.signal.aborted) {
                        const err = new Error("AbortError");
                        err.name = "AbortError";
                        throw err;
...
${formatted}`;
    ...
    async delayWithCountdown(attempt, delayMs) {
