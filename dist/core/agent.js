...
import { executeToolCall, getToolDescription, isDangerousCommand, MODIFYING_TOOLS, isToolCallOutOfBounds, isModelConfigAccess, isSensitiveEnvFileAccess, normalizeAndCheckSubpath, } from "./permissions.js";
...
export function checkPlanStructure(content) {
...
export const agentLocalStorage = new AsyncLocalStorage();
...
function isRetryableError(err) {
    ...
    let statusCode;
    if (err instanceof Error) {
        if (err.name === "AbortError" || err.message.toLowerCase().includes("aborted") || err.message.toLowerCase().includes("abort"))
            return false;
        msg = err.message;
...
export class Agent {
    ...
    planState = "IDLE";
    isSimpleTask = false;
    simpleTaskApproved = false;
    lastSpeed = null;
    goalMode = null;
    goalMaxIterations = 200;
    wasRunningBeforeAbort = false;
    allowSessionOutOfBounds = false;
     ...
     */
    static MAX_SKILL_LINES = 300;
    static MANDATORY_SKILLS = [
     ...
     */
    static trimSkillContent(raw, absolutePath) {
     ...
     */
    markPreloadedSkillsInList(skillsPrompt) {
        if (this.preloadedSkillKeys.size === 0)
            return skillsPrompt;
     ...
     */
    async answerQuestionAsMaster(question, options, context) {
        if (options.length === 0)
            return "";
...
If none of the options are suitable, still pick the closest one.`;
    ...
    constructor(onEvent, onPermission, onQuestion, customSystemPrompt, customTools, workingDirectory) {
     ...
     */
    emitViolation(reason, toolName, description, severity = "warning", meta) {
        const violation = {
            timestamp: Date.now(),
            reason,
            toolName,
    ...
    contextManagerInitFailed = false;
    getPlanFilePath() {
    ...
    resolveHistoryFilePath(autoResume) {
        ensureGlobalConfigDir();
    ...
    async loadHistory(autoResume = false) {
    ...
    async loadHistoryFromPath(filePath) {
...
Reply with EXACTLY "yes" if it is a simple task, or "no" if it is not. Reply with nothing else.`;
                ...
                if (classification === "yes" || classification.includes("yes")) {
                    ...
                    this.planState = "APPROVED";
                    const userInputText = typeof userInput === "string" ? userInput : userInput.map((p) => p.type === "text" ? p.text : "").join(" ");
...
${singleModeSubagentDirective}${goalModeAddendum}${guidelinesText}${processNotice}${pinnedKnowledgeNotice}${devHookNotice}`;
                        ...
                        try {
                            ...
                            for await (const delta of result.fullStream) {
                                ...
                                if (delta.type === "text-delta") {
                                    ...
                                    const reasoningText = delta.reasoning || delta.reasoningDelta || delta.delta || "";
                                    if (reasoningText) {
                ...
                for (const tc of toolCalls) {
                    ...
                    if (tc.name === "ask_question") {
                        if (Array.isArray(tc.args.questions) && tc.args.questions.length > 0) {
                            const normalizedQuestions = tc.args.questions.map((q) => {
                                const qObj = q;
                                const qOpts = Array.isArray(qObj.options) ? qObj.options.map((o) => {
                                    if (typeof o === "string")
                                        return o;
                        ...
                        let question = tc.args.question || "";
                        let rawOptionsVal = tc.args.options;
                        let isMultiSelect = tc.args.isMultiSelect;
                    ...
                    if (MODIFYING_TOOLS.includes(tc.name)) {
                        const filePath = tc.args.filePath || tc.args.file_path || tc.args.TargetFile || "";
                        ...
                        if (this.isSimpleTask && !this.simpleTaskApproved && !isPlanFile && !isTaskFile && !isWalkthroughFile) {
                            ...
                            try {
                                const selected = await this.onQuestion(`Agent is about to modify ${filename}. Proceed with modifications?`, ["Yes", "No"]);
                                if (selected === "Yes") {
                                    this.simpleTaskApproved = true;
                                }
                                else {
                        ...
                        if (isPlanFile) {
                            ...
                            if (tc.name === "write" || tc.name === "write_to_file") {
                                planContent = (tc.args.content || tc.args.codeContent || tc.args.CodeContent || "").trim();
                            }
                        ...
                        if (isTaskFile) {
                            let taskContent = "";
                            if (tc.name === "write" || tc.name === "write_to_file") {
                                taskContent = (tc.args.content || tc.args.codeContent || tc.args.CodeContent || "").trim();
                            }
                            else if (tc.name === "replace_file_content") {
                                const target = tc.args.TargetContent || "";
                                const replacement = tc.args.ReplacementContent || "";
                    ...
                    if (tc.name === "bash" || tc.name === "run_command" || tc.name === "run_background_process") {
                        if (this.planState === "PLANNING_PENDING") {
                            const cmd = (tc.args.command || "").trim();
                        ...
                        if (isDangerousCommand(tc.args.command) && !this.allowSessionDangerous) {
                            ...
                            if (approved === "session") {
                                ...
                                try {
                                    const { appendToolsErrorLog } = await import("./tools/state.js");
                                    appendToolsErrorLog(this.tier, this.delegationDepth, tc.name, denied.result, { command: tc.args.command, reason: "user_permission_denied" });
                                }
                                catch { }
                                this.emitViolation("user_permission_denied", tc.name, `Dangerous command denied by user/permission handler: "${tc.args.command}"`, "critical", { command: tc.args.command });
                    ...
                    if (needsPermission) {
                        ...
                        if (tc.args) {
                            ...
                            const targetPath = (tc.args.filePath ?? tc.args.file_path ?? tc.args.TargetFile ?? tc.args.path ?? tc.args.DirectoryPath ?? tc.args.SearchPath ?? tc.args.AbsolutePath);
                            const cwd = tc.args.cwd;
                            const detailsParts = [];
                            if (cmd)
                                detailsParts.push(`Command: "${cmd}"`);
                            if (targetPath)
                                detailsParts.push(`Target Path: "${targetPath}"`);
                            if (cwd)
                                detailsParts.push(`CWD: "${cwd}"`);
                        ...
                        const approved = await this.onPermission(tc, permMessage);
                    ...
                    const toolResult = await executeToolCall(tc, effectiveCwd, signal);
                    if (signal?.aborted) {
                        const err = new Error("AbortError");
                        err.name = "AbortError";
                        throw err;
                ...
                this.conversation.addAssistantMessage(textContent, toolCalls, toolResults);
                this.conversation.addMessage({
                    role: "tool",
                    content: "",
                    toolResults,
                ...
                if (i === maxIterations - 1) {
                    if (continueCount >= maxContinues) {
                        ...
                        try {
                            const selected = await this.onQuestion(`Reached maximum iteration limit of ${maxIterations} steps. The task may be incomplete. Would you like to continue for another ${maxIterations} steps?`, ["Yes, continue", "No, stop here"]);
    ...
    modelSupportsVision(modelName) {
        ...
        if (name.includes("gpt-4o"))
            return true;
        if (name.includes("gpt-4-vision"))
            return true;
        if (name.includes("gemini"))
            return true;
        if (name.includes("gemma-3"))
            return true;
    ...
    buildMessages() {
        ...
        for (const m of this.conversation.getMessages()) {
            ...
            if (m.role === "user") {
                ...
                const sdkContent = typeof m.content === "string"
                    ? m.content
                    : m.content.map((p) => {
                        if (p.type === "image") {
                            if (supportsVision) {
                ...
                for (let i = coreMessages.length - 1; i >= 0; i--) {
                    ...
                    if (prev.role === "assistant") {
                        if (Array.isArray(prev.content)) {
                            lastAssistantWithToolCalls = prev.content.some((part) => part.type === "tool-call");
                        }
                        break;
                    }
    ...
    async compactHistoryIfNeeded(signal) {
    ...
    async ensureContextManager() {
        if (this.conversation.hasContextManager())
            return;
        if (this.contextManagerInitFailed)
            return;
        try {
            ...
            this.writeToLogFile("WARN", `ContextManager init failed permanently: ${err.message}`);
        }
    }
    async contextManagerCompact(signal) {
        const contextManager = this.conversation.getContextManager();
        if (signal) {
        ...
        try {
            this.writeToLogFile("INFO", `Context compaction triggered: ${decision.reason} (strategy: ${decision.recommendedStrategy?.name || "auto"})`);
            const result = await contextManager.compact(messages, undefined, signal);
            this.conversation.replaceMessages(result.messages);
            await this.saveHistory();
            this.writeToLogFile("INFO", `Compaction completed: ${result.metadata.strategy} strategy, ${result.metadata.messagesBefore || 0} -> ${result.metadata.messagesAfter || 0} messages`);
        }
        catch (error) {
            console.error("ContextManager compaction failed:", error);
            this.writeToLogFile("ERROR", `ContextManager compaction failed: ${error.message}`);
            await this.legacyCompactHistory(signal);
        }
    }
    async legacyCompactHistory(signal) {
        const modelLimit = getContextWindowLimit(this.config.model);
    ...
    async summarizeMessages(messages, signal) {
...
${formatted}`;
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    const timeout = setTimeout(() => {
                        if (summarySignal)
                            summarySignal.removeEventListener("abort", onAbort);
                        resolve();
    ...
    async delayWithCountdown(attempt, delayMs, signal) {
     ...
     */
    autoCheckpoint(label) {
        const now = Date.now();
        if (now - this.lastAutoCheckpointAt < Agent.AUTO_CHECKPOINT_COOLDOWN_MS)
            return;
    ...
    async prepopulateTencentDBMemoryContext() {
        const messages = this.conversation.getMessages();
        // Check if we already have a memory context message in the conversation
        const hasMemoryContext = messages.some((m) => m.role === "user" && contentToString(m.content).startsWith("[TencentDB Agent Memory Context]:"));
        if (hasMemoryContext)
            return;
        // Fetch the memories
    ...
    async syncConversationToTencentDB() {
        const settings = getSettings();
        if (!settings.enableTencentdbMemory)
            return;
