...
export function useDashboardWizard(ctx) {
    ...
    const getTierOptionsList = (masterModelFormatted, superagentModelFormatted, subagentModelFormatted, researcherModelFormatted, coderModelFormatted, reviewerModelFormatted) => {
        if (isMulti) {
            return [
    ...
    const getPresetOptionsList = (models) => {
        const formatVal = (val) => val ? val : "(not set)";
        if (isMulti) {
            return [
    ...
    const handleWizardSubmit = useCallback(async (value) => {
        if (!activeWizard)
            return;
        const now = Date.now();
...
[Add 2-3 specific custom guidelines for the target stack if helpful]`;
                        ...
                        const modelConfig = agent.getModel();
            ...
            if (activeWizard.step === 1) {
                ...
                if (value.toLowerCase().includes("not set") || value === "6") {
                    ...
                    let targetLabel = "";
                    const cMode = isMulti ? "multi" : "single";
                    if (tier === "master") {
                        if (isMulti)
                            clearTierModel(cMode, "master");
                        ...
                        fetch("https://openrouter.ai/api/v1/models", { headers: { Authorization: `Bearer ${resolvedApiKey}` } })
                            ...
                            if (res.ok) {
                                ...
                                if (data && Array.isArray(data.data)) {
                                    setWizardOptions([...data.data.map((m) => m.id), "< Back"]);
                                }
                            }
                        })
                            .catch(() => { })
                ...
                try {
                    ...
                    if (tier === "default") {
                        ...
                        const finalModelName = profileName.toLowerCase() !== activeProvider.toLowerCase()
                            ? `${profileName.toLowerCase()}@${modelName}`
                            : modelName;
                        const wMode = isMulti ? "multi" : "single";
                        if (tier === "master") {
                            if (isMulti)
                                setTierModel(wMode, "master", finalModelName);
                ...
                try {
                    ...
                    if (isSingle) {
                        ...
                        updatedLogs.push(`[MASTER]   Single Agent Model: ${singleModel}`, `[MASTER]   Subagent (depth 2): ${subagentModel}`);
                        ...
                        updatedLogs.push(`[MASTER]   Master Agent (depth 0): ${masterModel}`, `[MASTER]   Superagent (depth 1): ${superagentModel}`, `[MASTER]   Subagent (depth 2): ${subagentModel}`);
                ...
                const models = activeWizard.data.presetModels ? JSON.parse(activeWizard.data.presetModels) : {};
                if (value === "< Back") {
                ...
                let tier = "";
                if (value.includes("Master Agent"))
                    tier = "master";
                else if (value.includes("Superagent"))
                    tier = "superagent";
                else if (value.includes("depth 2"))
                    tier = "subagent";
                else if (value.includes("researcher"))
                    tier = "researcher";
                else if (value.includes("coder"))
                    tier = "coder";
                else if (value.includes("reviewer"))
                    tier = "reviewer";
                else if (value.includes("Default Model"))
                    tier = "default";
                if (!tier)
                    return;
                ...
                if (value.toLowerCase().includes("not set") || value === "6") {
                    const tier = activeWizard.data.tier || "";
                    const presetModels = activeWizard.data.presetModels ? JSON.parse(activeWizard.data.presetModels) : {};
                    if (tier === "master") {
                        ...
                        if (isMulti) {
                            ...
                            delete presetModels.MODEL_SINGLE_SUPERAGENT;
                        }
                    }
                    else if (tier === "subagent") {
                        if (isMulti) {
                ...
                try {
                    ...
                    if (tier === "default") {
                        ...
                        const finalModelName = provider.toLowerCase() !== activeProvider.toLowerCase()
                            ? `${provider.toLowerCase()}:${selectedModel}`
                            : selectedModel;
                        const wMode2 = isMulti ? "multi" : "single";
                        if (tier === "master") {
                            if (isMulti)
                                setTierModel(wMode2, "master", finalModelName);
                            targetLabel = isMulti ? "Master Agent (depth 0) Model" : "Single Agent Model";
                        }
                        else if (tier === "superagent") {
                    ...
                    updatedLogs.push(`[MASTER]   Superagent (depth 1): ${superagentModel}`, `[MASTER]   Subagent (depth 2): ${subagentModel}`);
            ...
            if (activeWizard.step === 1) {
                ...
                if (wizardSelectedIndex === 0) {
                    ...
                    setIsProcessing(true);
                    agent.sendMessage(`I would like you to use the following skill: "${chosen.name}".\nPlease read its instruction file at "${chosen.path}" using a file read tool first, and then help me with my request based on its instructions.`).then(() => {
                        setIsProcessing(false);
                    }).catch((err) => {
                        setIsProcessing(false);
    ...
    const handleQuerySubmit = useCallback((val) => {
        setIsPasted(false);
        ...
        if (!cleanVal && attachments.length === 0)
            return;
        if (cleanVal) {
            setHistory((prev) => {
        ...
        if (commandInput.startsWith("/")) {
            ...
            const handleAttachImage = async (filePath) => {
                try {
        ...
        setIsProcessing(true);
        let messageContent = commandInput;
        if (attachments.length > 0) {
            const parts = [
                ...(commandInput ? [{ type: "text", text: commandInput }] : []),
...
//# sourceMappingURL=useDashboardWizard.js.map