...
export function useKeyboardHandler(ctx) {
    ...
    handlerRef.current = (inputChar, key) => {
        ...
        if (key.ctrl && inputChar === "t" && !activeWizard) {
            if (planState === "APPROVED" && (checklistTasks.length > 0 || completedHistory.length > 0)) {
                setFocusMode((prev) => (prev === "checklist" ? "input" : "checklist"));
            }
            return;
        }
            ...
            if (activeWizard.type === "login" && (activeWizard.step === 1 || activeWizard.step === 2 || activeWizard.step === 6 || activeWizard.step === 7 || activeWizard.step === 8 || activeWizard.step === 10)) {
                ...
                if (key.return) {
                    ...
                    if (choice === "preset_load") {
                        const presetMode = agentRef.current?.isMultiAgent ? "multi" : "single";
                        setActiveWizard({
                            type: "model",
                            step: 4,
                            data: { tier: choice },
                    ...
                    if (choice === "configure_tiers") {
                        const kbMode = agentRef.current?.isMultiAgent ? "multi" : "single";
                ...
                if (key.leftArrow) {
                    setActiveWizard((curr) => curr ? { ...curr, data: { ...curr.data, focus: "plan" } } : null);
                    return;
                }
                if (key.rightArrow) {
                    setActiveWizard((curr) => curr ? { ...curr, data: { ...curr.data, focus: "actions" } } : null);
                    return;
                }
                ...
                if (activeWizard.step === 1) {
                    ...
                    if (key.return) {
                        ...
                        if (!selectedOption || selectedOption === "(no results)")
                            return;
                    ...
                    if (key.return) {
                        ...
                        if (wizardSelectedIndex === 0) {
                            ...
                            agentRef.current?.sendMessage(`I would like you to use the following skill: "${chosen.name}".\nPlease read its instruction file at "${chosen.path}" using a file read tool first, and then help me with my request based on its instructions.`).catch((err) => {
        ...
        if (isEscape) {
            if (scrollOffset > 0) {
                ...
                if (activeWizard.type === "model" && activeWizard.step !== 1) {
                    ...
                    setActiveWizard({ type: "checkpoint", step: 1, data: { action: "browse" } });
                    const listOptions = checkpointsList.map((c) => `${c.name} (${new Date(c.timestamp).toLocaleString()}) - ${c.messages.length} messages`);
    ...
    const stableHandler = useCallback((inputChar, key) => {
        handlerRef.current?.(inputChar, key);
    }, []);
    useInput(stableHandler);
    // Small y/n listener for permission wizard
    const permHandlerRef = useRef();
    ...
    const stablePermHandler = useCallback((inputChar, key) => {
        permHandlerRef.current?.(inputChar);
    }, []);
    useInput(stablePermHandler, { isActive: activeWizard?.type === "permission" });
}
