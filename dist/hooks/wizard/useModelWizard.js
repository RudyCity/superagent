...
import { getConfiguredProviders, getProviders, switchActiveProvider, fetchAndCacheModels, getContextWindowLimit, getModelPresets, applyModelPreset, saveModelPreset, deleteModelPreset, BUILT_IN_PRESETS, getProviderOptionsList, addProvider, getActiveProviderName, getResolvedModelWithProvider, formatProviderForPicker, getEffectiveMasterModel, getTierModel, getTierModelWithProvider, setTierModel, setAllTierModels, clearTierModel, getAllTierModels } from "../../core/config.js";
import { getDefaultModel } from "../../core/slash-commands.js";
export function useModelWizard(ctx) {
    const { setActiveWizard, setWizardOptions, setWizardSelectedIndex, addLine, setInput, setContextLimit, setActiveModel, setWizardIsLoadingModels, wizardSelectedIndex, wizardOptions, wizardIsLoadingModels, agentRef, } = ctx;
    const handleModelWizard = useCallback(async (value, step, data) => {
        const now = Date.now();
         ...
         */
        const syncContextManagerModel = (modelName, limit) => {
        ...
        if (step === 1) {
            ...
            const models = data.presetModels ? JSON.parse(data.presetModels) : {};
            if (value === "< Back") {
            ...
            if (value.includes("Master Agent"))
                ...
                tier = "default";
            else if (value.includes("Single Agent"))
                tier = "single";
            if (!tier)
                return;
...
//# sourceMappingURL=useModelWizard.js.map