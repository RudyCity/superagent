...
export function getDashboardSuggestions(query) {
    if (!query.startsWith("/"))
        return [];
...
export function getSuggestionDescriptions() {
    const desc = { ...BUILTIN_DESCRIPTIONS };
    // Auto-populate descriptions from registry for any command without a manual entry
    for (const cmd of registry.getAll()) {
        const key = `/${cmd.name}`;
        if (!desc[key] && cmd.description)
            desc[key] = cmd.description;
        if (cmd.aliases) {
            for (const alias of cmd.aliases) {
                const aliasKey = `/${alias}`;
                if (!desc[aliasKey] && cmd.description)
                    desc[aliasKey] = cmd.description;
            }
        }
    }
