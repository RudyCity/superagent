 ...
 */
export async function readImageFromPath(filePath) {
...
async function readClipboardLinux() {
    ...
    // wl-paste (Wayland)
    try {
        const { stdout } = await execFileAsync("wl-paste", ["--type", "image/png", "--no-newline"], { timeout: 5000, encoding: "buffer" });
    ...
    // xclip (X11)
    try {
        const { stdout } = await execFileAsync("xclip", ["-selection", "clipboard", "-t", "image/png", "-o"], { timeout: 5000, encoding: "buffer" });
        if (stdout && stdout.byteLength > 0) {
            const buf = stdout;
            return {
                id: crypto.randomUUID(),
    ...
    try {
        await execFileAsync("xsel", ["--clipboard", "--output"], { timeout: 5000 });
 ...
 */
export function attachmentToImagePart(attachment) {
    return {
        type: "image",
