...
if (!apiKey && !hasCustomEndpoint && !process.stdin.isTTY) {
    console.error("Error: No API key configured. Run superagent and use /login add to configure a provider.");
    process.exit(1);
}
...
if (process.stdin.isTTY) {
    ...
    const confirmTrust = async (dir) => {
        const { TrustPrompt } = await import("./components/trust-prompt.js");
        return new Promise((resolve) => {
            const { unmount } = render(React.createElement(TrustPrompt, {
                directoryPath: dir,
                onAccept: () => {
                    unmount();
                    resolve(true);
    ...
    if (isMulti) {
        ...
        const questionHandlerRef = { current: null };
        ...
        const { waitUntilExit } = render(React.createElement(MultiAgentDashboard, {
            agent,
            autoResume,
