...
function getWizardBorderColor(activeWizard) {
    if (!activeWizard)
        return "cyan";
...
export function App({ autoResume = false, onHistoryChange, onSessionPath, initialPrompt, }) {
    ...
    const setLines = useCallback((val) => {
        _setLines((prev) => {
    ...
    const toggleLineExpand = useCallback((index) => {
    ...
    const toggleChildExpand = useCallback((parentIndex, childIndex) => {
        setExpandedChildren(prev => {
            const next = new Map(prev);
    ...
    const addToolChild = useCallback((child) => {
        setLines((prev) => {
     ...
     */
    const patchLastToolStart = useCallback((result) => {
        setLines((prev) => {
            for (let i = prev.length - 1; i >= 0; i--) {
                if (prev[i].type === "assistant") {
    ...
    const scrollChat = useCallback((direction, amount = 1) => {
    ...
    const openResponseAtIndex = useCallback((index) => {
    ...
    const handleSubmit = useCallback(async (value) => {
        ...
        if (activeWizard) {
            ...
            setHistoryIndex(-1);
            setScrollOffset(0);
            const isSelectionStep = (activeWizard.type === "login" && (activeWizard.step === 1 || activeWizard.step === 2 || activeWizard.step === 6 || activeWizard.step === 7 || activeWizard.step === 8 || activeWizard.step === 10)) ||
        ...
        setHistoryIndex(-1);
        setScrollOffset(0);
        const runInteractiveProcess = async (command, cwd, env) => {
    ...
    const commands = [
        ...new Set(registry.getAll().flatMap(cmd => {
            const names = [`/${cmd.name}`];
            if (cmd.aliases)
                names.push(...cmd.aliases.map(a => `/${a}`));
            return names;
        })),
        ...skillCommands
    ];
    ...
    const getWizardQuestion = () => {
        ...
        if (activeWizard.type === "login") {
            if (activeWizard.step === 1)
                return "Select whether to view configured providers or create a new one.";
            if (activeWizard.step === 2)
                return "Select the provider type to configure.";
            if (activeWizard.step === 3)
                return "Enter a name for the provider profile (or press Enter for default).";
            if (activeWizard.step === 4)
                return "Enter the custom endpoint base URL.";
            if (activeWizard.step === 5)
                return "Paste the API key for this provider.";
            if (activeWizard.step === 6)
                return "Select a provider from the configured list.";
            if (activeWizard.step === 7)
                return "Select whether to test the provider connection first.";
            if (activeWizard.step === 8)
                return "Select an available model (type to filter).";
            if (activeWizard.step === 9)
                return "Type a test message to send to the model.";
            if (activeWizard.step === 10)
                return "Select the technology stack for the new project.";
            if (activeWizard.step === 11)
                return "Enter the project name (or press Enter for folder default).";
        ...
        if (activeWizard.type === "model") {
            if (activeWizard.step === 1)
                return "Select model configuration option.";
            if (activeWizard.step === 2)
                return "Select provider for model tier.";
            if (activeWizard.step === 3)
                return "Select a configured provider profile, or create a new one.";
            if (activeWizard.step === 6)
                return "Enter a name for the new provider profile.";
            if (activeWizard.step === 7)
                return "Enter the custom endpoint base URL.";
            if (activeWizard.step === 8)
                return "Paste the API key for the new provider profile.";
    ...
    const getWizardPlaceholder = () => {
        ...
        if (activeWizard.type === "skills") {
            if (activeWizard.step === 1)
                return "🔍 Search skills (type to filter, arrows to navigate, Enter to select)...";
            if (activeWizard.step === 2)
                return "Select action using arrows and Enter (Esc: Back)...";
        }
        if (activeWizard.type === "login") {
            if (activeWizard.step === 1)
                return "Select option using arrows and Enter (Esc: Cancel)...";
            if (activeWizard.step === 2)
                return "Select provider template using arrows and Enter (Esc: Back)...";
            if (activeWizard.step === 3)
                return "Enter config profile name (or press Enter for default, Esc: Back)...";
            if (activeWizard.step === 4)
                return "Enter Custom Base URL (Esc: Back)...";
            if (activeWizard.step === 5)
                return "Paste API key (Esc: Back)...";
            if (activeWizard.step === 6)
                return "Select provider using arrows and Enter (Esc: Cancel)...";
            if (activeWizard.step === 7)
                return "Select option using arrows and Enter (Esc: Back)...";
            if (activeWizard.step === 8)
                return "🔍 Search models (type to filter, arrows to navigate, Enter to select)...";
            if (activeWizard.step === 9)
                return "Type your test message and press Enter...";
            if (activeWizard.step === 10)
                return "Select option using arrows and Enter (Esc: Cancel)...";
            if (activeWizard.step === 11)
                return "Enter project name (press Enter for folder default, Esc: Back)...";
            if (activeWizard.step === 12)
        ...
        if (activeWizard.type === "model") {
            if (activeWizard.step === 1)
                return "Select option using arrows and Enter...";
            if (activeWizard.step === 2)
                return "Enter provider number or select using arrows...";
            if (activeWizard.step === 20)
                return "Type preset name and press Enter (or type 'back' to go back)...";
            if (activeWizard.step === 21)
                return "Type preset description and press Enter (or type 'back' to go back)...";
            if (activeWizard.step === 22 || activeWizard.step === 32)
                return "Select tier option using arrows and Enter...";
            if (activeWizard.step === 23 || activeWizard.step === 33)
                return "Select provider using arrows and Enter...";
            if (activeWizard.step === 3 || activeWizard.step === 25 || activeWizard.step === 35)
                return "Select profile using arrows and Enter...";
            if (activeWizard.step === 15 || activeWizard.step === 24 || activeWizard.step === 34)
                return "🔍 Search models (type to filter, arrows to navigate, Enter to select)...";
            if (activeWizard.step === 30)
                return "Select preset to edit using arrows and Enter...";
            if (activeWizard.step === 31)
                return "Type new description and press Enter (or type 'back' to go back)...";
            if (activeWizard.step === 40)
                return "Select preset to delete using arrows and Enter...";
            if (activeWizard.step === 41)
                return "Select confirmation using arrows and Enter...";
            if (activeWizard.step === 50)
                return "Select agent tier using arrows and Enter...";
            if (activeWizard.step === 6)
                return "Enter config profile name (or press Enter for default)...";
            if (activeWizard.step === 7)
                return "Enter Custom Base URL...";
            if (activeWizard.step === 8)
        ...
        if (activeWizard.type === "question") {
            if (activeWizard.step === 2)
                return "Type custom answer and press Enter...";
    ...
    const permissionHandler = useCallback((toolCall, description) => {
    ...
    useEffect(() => {
        const unsubTasks = subscribeToTasks(() => {
            ...
            setRunningTasksCount(allTasks.filter((t) => !t.hasExited).length);
            allTasks.forEach((task) => {
                if (task.isDetachedWindow)
                    return;
                if (task.hasExited && !task.notified) {
                    task.notified = true;
        ...
        const unsubSubagents = subscribeToSubagents(() => {
            setRunningSubagentsCount(Array.from(subagentInstances.values()).filter((s) => s.status === "running").length);
            const activeList = Array.from(subagentInstances.values());
            activeList.forEach((inst) => {
                if (inst.status === "completed" && inst.result && !inst.notified) {
                    inst.notified = true;
        ...
        const unsubSuperagents = subscribeToSuperagents(() => {
            setRunningSuperagentsCount(Array.from(superagentInstances.values()).filter((s) => s.status === "running").length);
    ...
    if (activeWizard) {
        if (activeWizard.type === "plan_approve") {
            ...
            if (activeWizard.type === "login") {
                if (activeWizard.step === 1 || activeWizard.step === 2)
                    wizardSectionHeight += 8;
                else if (activeWizard.step === 10)
                    wizardSectionHeight += 8 + Math.min(6, wizardOptions.length);
                else if ([3, 4, 6, 11, 12, 13].includes(activeWizard.step))
                    wizardSectionHeight += 6;
            }
            else if (activeWizard.type === "model") {
    ...
    return (_jsxs(Box, { flexDirection: "column", height: terminalHeight, children: [_jsx(Box, { flexDirection: "row", flexGrow: 1, children: _jsxs(Box, { flexDirection: "column", width: "100%", flexGrow: 1, children: [_jsx(ChatArea, { showBanner: showBanner, focusMode: focusMode, scrollOffset: scrollOffset, focusedResponseIndex: focusedResponseIndex, setFocusedResponseIndex: setFocusedResponseIndex, focusedResponseOffset: focusedResponseOffset, setFocusedResponseOffset: setFocusedResponseOffset, lines: lines, chatHeightLimit: chatHeightLimit, terminalHeight: terminalHeight, terminalWidth: terminalWidth, isProcessing: isProcessing, streamDisplay: streamDisplay, tokensUp: tokensUp, tokensDown: tokensDown, liveStreamTokens: liveStreamTokens, modelName: activeModel, maxAssistantResponseLines: 12, isExecutingTool: isExecutingTool, timeLeft: timeLeft, activeToolOutput: activeToolOutput, formatCompactNumber: formatCompactNumber, onVisibleLinesChange: setVisibleLinePositions, chatContentStartRow: chatContentStartRow, expandedLines: expandedLines, toggleLineExpand: toggleLineExpand, expandedChildren: expandedChildren, toggleChildExpand: toggleChildExpand, wrappedLines: wrappedLines }), _jsxs(Box, { flexDirection: "column", paddingX: 1, marginTop: 1, flexShrink: 0, children: [_jsx(ActiveAgentsList, { focusMode: focusMode, runningSuperagentsCount: runningSuperagentsCount, runningSubagentsCount: runningSubagentsCount, runningTasksCount: runningTasksCount, superagentsScrollOffset: superagentsScrollOffset, subagentsScrollOffset: subagentsScrollOffset, procsScrollOffset: procsScrollOffset, maxSuperagentsVisible: maxSuperagentsVisible, maxSubagentsVisible: maxSubagentsVisible, maxProcsVisible: maxProcsVisible, collapsedSections: collapsedSections }), _jsx(TaskChecklist, { planState: planState, checklistTasks: checklistTasks, checklistScrollOffset: checklistScrollOffset, maxChecklistVisible: maxChecklistVisible, focusMode: focusMode, isMultiAgent: !!agentRef.current?.isMultiAgent, completedHistory: completedHistory }), _jsx(HistoryPanel, { history: history, historySelectedIndex: historySelectedIndex, focusMode: focusMode }), _jsx(WizardPanels, { activeWizard: activeWizard, wizardOptions: wizardOptions, wizardSelectedIndex: wizardSelectedIndex, wizardSelectedSet: wizardSelectedSet, pendingPermission: pendingPermission, pendingQuestion: pendingQuestion, planState: planState, planUrl: planUrl, planFilePath: planPath, input: input, wizardIsLoadingModels: wizardIsLoadingModels, checkpointsList: checkpointsList, goalMode: goalMode, suggestions: suggestions, focus: activeWizard?.data?.focus || "actions", scrollOffset: parseInt(activeWizard?.data?.scrollOffset || "0", 10), onScrollChange: (offset) => setActiveWizard((curr) => curr ? { ...curr, data: { ...curr.data, scrollOffset: String(offset) } } : null) }), !isSelectionOnlyStep && (_jsxs(Box, { flexDirection: "column", children: [(() => {
                                            ...
                                            if (!question)
                                                return null;
                                            return (_jsxs(Box, { flexDirection: "row", marginBottom: 1, children: [_jsx(Text, { color: activeWizard ? getWizardBorderColor(activeWizard) : "green", children: "\u2502 " }), _jsx(Text, { color: "cyan", wrap: "truncate-end", children: question })] }));
                                        })(), _jsxs(Text, { color: scrollOffset > 0 ? "yellow" : activeWizard ? getWizardBorderColor(activeWizard) : isProcessing ? "gray" : "green", children: ["\u2514\u2500\u2500\u2500[ ", _jsx(Text, { bold: true, color: scrollOffset > 0 ? "yellow" : activeWizard ? getWizardBorderColor(activeWizard) : isProcessing ? "gray" : "green", children: activeWizard ? `⚙️ WIZARD: ${activeWizard.type.toUpperCase()} (Step ${activeWizard.step})` : "⌨️ COMM_LINK: ACTIVE" }), " ]", isProcessing && displayPrompt && (_jsxs(Text, { color: "cyan", bold: true, children: [" \u2500\u2500\u2500 [ PROMPT: \"", displayPrompt, "\" ]"] }))] }), _jsxs(Box, { flexDirection: "row", children: [_jsx(Text, { color: activeWizard ? getWizardBorderColor(activeWizard) : isProcessing ? "gray" : "green", children: "\u2502 \u276F " }), isProcessing && !activeWizard ? (_jsx(ProcessingIndicator, { scrollOffset: scrollOffset })) : (() => {
